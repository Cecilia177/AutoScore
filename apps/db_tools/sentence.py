...
class Sentence:
    ...
    Attributes:
        ...
        language -- "ch" or "en", defines the language.
        ...
        keywords -- A list of words in the sentence after removing stop words and lemmatizing.
    """
    ...
    def segment(self):
        ...
        if self.original == "":
            ...
            for s in sub_sentences:
                if s:
                    segments = " ".join(jieba.cut(s)) if self.language == "ch" else s
    ...
    def get_key_words(self):
        if self.language == 'en':
            with open("apps/db_tools/files/stopwords_en.txt", "r") as f:
                stopwords = f.read().split("\n")
            source_words = [i for i in self.pure_text.split(" ") if i and i not in stopwords]
            lemmatizer = WordNetLemmatizer()
            keywords = []
            for w in source_words:
                for p in ['a', 's', 'r', 'n', 'v']:
                    w1 = lemmatizer.lemmatize(w, pos=p)
                    if w1 == w and p == 'v':
                        keywords.append(w1)
                    elif w1 != w:
                        keywords.append(w1)
            ...
            self.keywords = [i for i in self.pure_text.split(" ") if i]
